...
class ModifyMenuApplicationService(ApplicationService[ModifyMenuDto, str]):
    def __init__(self, menus_repository: MenuRepository, event_handler: EventHandler) -> None:
    ...
    async def execute(self, input: ModifyMenuDto) -> Awaitable[Result[str]]:
        ...
        if menu is None:
            return Result[str].failure(error=PlateNotFoundApplicatonError(PlateId(input.id)))
